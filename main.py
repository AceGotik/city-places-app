...
def get_places_with_vote(
    ...
    for place in places:
        ...
        # достаём дополнительные фото и меню
        place_photos = db.query(PlacePhoto).filter(PlacePhoto.place_id == place.id).all()
        menu_photos = db.query(MenuPhoto).filter(MenuPhoto.place_id == place.id).all()
        ...
        result.append({
            ...
            "work_time": place.work_time,
            "rating": place.rating,
            "image": place.image,           # main cover (string или None)
            "latitude": place.latitude,
            "longitude": place.longitude,
            "user_vote": user_vote,
            ...
            "created_at": place.created_at,
            "photos": [{"id": p.id, "image": p.image} for p in place_photos],
            "menu": [{"id": m.id, "image": m.image} for m in menu_photos],
        })
